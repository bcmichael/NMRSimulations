"""
    propagate(pulse,start,prop_dict,parameters,temp)

Fetch the propagator corrseponding to a given 'pulse' and 'start' step from
'prop_dict'. Return the propagator and the number of steps used.
"""
function propagate(pulse::Pulse, start, prop_dict, parameters, temp)
...
"""
    propagate(block,start,prop_dict,parameters,temp)

Generate a propagator for the entire 'block' including its repeats using the
propagators in 'prop_dict'. Return the propagator and the number of steps used.
"""
function propagate(block::Block, start,prop_dict, parameters, temp)
    step_total = 0
    Uelement,steps,temp = propagate(block.pulses[1], start+step_total, prop_dict, parameters, temp)
    U = copy(Uelement)
    step_total += steps
    for j = 1:block.repeats
        iter = j == 1 ? 2 : 1
        for n = iter:length(block.pulses)
            Uelement, steps, temp = propagate(block.pulses[n], start+step_total, prop_dict, parameters, temp)
            mul!(temp, Uelement,U)
            step_total += steps
            U, temp = temp, U
        end
...
"""
    propagate_before_loop(sequence,loop,start_step,prop_dict,parameters,temp)
...
'sequence'.
"""
function propagate_before_loop(sequence, loop, start_step, prop_dict, parameters, temp)
    U = similar(temp)
    fill_diag!(U, 1)
    nonloop_steps = 0
    ...
    for n = start_iter:end_iter
        U1, steps, temp = propagate(sequence.pulses[n], nonloop_steps+start_step, prop_dict, parameters, temp)
        mul!(temp, U1, U)
        U, temp = temp, U
        nonloop_steps += steps
    end
...
function build_looped(sequence::Sequence, loop, old_loop_steps, old_nonloop_steps, prop_dict, parameters, temp::A) where {A}
    period_steps = parameters.period_steps
    step_size = parameters.step_size
    ...
    for n = 1:old_loop_cycle
        ...
        U, nonloop_steps, temp = propagate_before_loop(sequence, loop, start, prop_dict, parameters, temp)
        push!(Unonlooped, U)
    end
    ...
    for n = 1:old_loop_cycle
        for j = 1:loop_cycle
            ...
            Uloop,_,temp = propagate(loop_element, start+(n-1)*loop_steps, prop_dict, parameters, temp)
            Uloops[n,j] = Uloop
        end
    end
    Ustart = Vector{A}()
...
function build_first_looped(sequence::Sequence, prop_dict, parameters, temp::A) where {A}
    period_steps = parameters.period_steps
    step_size = parameters.step_size

    U, nonloop_steps, temp=propagate_before_loop(sequence, 1, 1, prop_dict, parameters, temp)
    nonloop_steps += 1
    initial = [U]
    ...
    Uloops = Vector{A}()
    for n = 1:loop_cycle
        Uloop, _, temp = propagate(loop_element, nonloop_steps+(n-1)*loop_steps, prop_dict, parameters, temp)
        push!(Uloops, Uloop)
    end
...
function build_nonlooped(sequence::Sequence, loop, old_loop_steps, old_nonloop_steps, prop_dict, parameters, temp::A) where {A}
    period_steps = parameters.period_steps
...
function find_pulses!(prop_dict, block::Block, start, parameters)
    step_total = 0
    for j = 1:block.repeats
        for n = 1:length(block.pulses)
            step_total += find_pulses!(prop_dict, block.pulses[n], start+step_total, parameters)
        end
    end
    return step_total
end

function find_pulses!(prop_dict, sequence::Sequence, start, parameters)
    detection_loop = sequence.detection_loop
    issorted(detection_loop) || error("detection loop must be sorted")

    loop_steps, nonloop_steps = find_first_looped!(prop_dict, sequence, parameters)
    for n = 2:length(detection_loop)
        loop_steps, nonloop_steps = find_looped!(prop_dict, sequence, n, loop_steps, nonloop_steps, parameters)
    end

    if detection_loop[end] < length(sequence.pulses)
        loop_steps, nonloop_steps = find_nonlooped!(prop_dict, sequence, n, loop_steps, nonloop_steps, parameters)
    end
    return prop_dict
end

function find_before_loop!(prop_dict, sequence, loop, start_step, parameters)
    nonloop_steps = 0
    ...
    for n = start_iter:end_iter
        nonloop_steps += find_pulses!(prop_dict, sequence.pulses[n], nonloop_steps+start_step, parameters)
    end

    return nonloop_steps
end

function find_first_looped!(prop_dict, sequence, parameters)
    period_steps = parameters.period_steps
    step_size = parameters.step_size

    nonloop_steps = find_before_loop!(prop_dict, sequence, 1, 1, parameters)
    nonloop_steps += 1
    ...
    for n = 1:loop_cycle
        ...
        find_pulses!(prop_dict, loop_element, start, parameters)
    end

    return loop_steps, nonloop_steps
end

function find_looped!(prop_dict, sequence, loop, old_loop_steps, old_nonloop_steps, parameters)
    period_steps = parameters.period_steps
    step_size = parameters.step_size
    ...
    for n = 1:old_loop_cycle
        start = old_nonloop_steps+(n-1)*old_loop_steps
        nonloop_steps = find_before_loop!(prop_dict, sequence, loop, start, parameters)
    end
    loop_element = sequence.pulses[sequence.detection_loop[loop]]
    loop_steps = Int(duration(loop_element)/step_size)
    loop_cycle = div(lcm(loop_steps, period_steps), loop_steps)
    ...
    for n = 1:old_loop_cycle
        for j = 1:loop_cycle
            start = old_nonloop_steps+nonloop_steps+(n-1)*old_loop_steps+(j-1)*loop_steps
            find_pulses!(prop_dict, loop_element, start+(n-1)*loop_steps, parameters)
        end
    end
    return old_loop_steps+loop_steps, old_nonloop_steps+nonloop_steps
end

function find_nonlooped!(prop_dict, sequence, loop, old_loop_steps, old_nonloop_steps, parameters)
    period_steps = parameters.period_steps
